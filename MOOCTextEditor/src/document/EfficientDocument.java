...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		this.numWords = 0;
		this.numSentences = 0;
		if (!this.getText().endsWith("!") & !this.getText().endsWith("?") & !this.getText().endsWith(".") & listSize!=0 )
          this.numSentences = 1;		
		
		if (listSize != 0)  {
		for (int i = 0; i<listSize; i++){
			this.numSyllables = this.numSyllables + this.countSyllables(tokens.get(i));
			if (this.isWord(tokens.get(i)))
				this.numWords++;
