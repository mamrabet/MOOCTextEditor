...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index <= s.length(); index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++)
			{
				...
				x.insert(index, (char)charCode );
			   if(wordsOnly && dict.isWord(x.toString()))
				  currentList.add(x.toString());
			   else if (!wordsOnly)
				   currentList.add(x.toString());
			}
		}
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index < s.length(); index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++)
			{
				...
				x = s.substring(0, index) + s.substring(index+1);
				
				if(wordsOnly && dict.isWord(x))
					  currentList.add(x);
				   else if (!wordsOnly)
					   currentList.add(x);
			}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (!queue.isEmpty() && i<numSuggestions)
	    	{
			curr = queue.remove(0);
		    subsitution(curr,neighborList,true);
		    insertions(curr,neighborList,true);
		    deletions(curr,neighborList,true);
